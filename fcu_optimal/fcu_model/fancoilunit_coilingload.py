...
cls                         系统需求冷负荷             W 瓦
tw1                         进水温度                ℃ 摄氏度
V_air                       循环风量                m3/s 立方米每秒
W_water                     进水的质量流量           kg/s 千克每秒
t_in_dryair                 进风干球温度             ℃摄氏度
t_in_relative_humidity      进风相对湿度             无量纲
输出
cls                         系统需求冷负荷             W 瓦
Q2                          制冷量                   W 瓦
t2                          出风干球温度             ℃ 摄氏度
d2                          含湿量                   kg/kg_air
tw2                         出水温度                ℃ 摄氏度
W_water*3600                进水的质量流量           kg/h 千克每小时
V_air*3600                  循环风量                kg/h 千克每小时
Ppump                       水泵功耗                W 瓦
Pfan                        风机功耗                水泵功耗
'''
...
# HAPropsSI 用来计算湿空气相关参数（已知两个参数，能求解其他参数）
...
class physical_parameter_water:
    ...
    def ydnd(self):
        test_ydnd=np.load("FCU_control/ydnd.npy")
    ...
    def drl(self):
        test_drl=np.load("FCU_control/drl.npy")
    ...
    def dlnd(self):
        test_dlnd=np.load("FCU_control/dlnd.npy")
...
class fan_coil_unit:
    def __init__(self,
        ...
        self.tw1=tw1    #进水温度
        self.cls=cls   # 系统冷负荷
        self.V_air=V_air#循环风量 m3/s （可控的） （风的动作）
        self.W_water=W_water#(水的质量流量 kg/s) （水的动作）
        self.t_in_dryair=t_in_dryair#(进风干球温度 °C) （读取过来的）
        self.t_in_relative_humidity=t_in_relative_humidity#（进风相对湿度 无量纲）（读取过来的）
        self.delta_f=delta_f#翅片厚度 m
        self.delta_t=delta_t#管壁厚度 m
        self.do=do#管子外径 m
        self.di=di#管子内径 m
        self.s1=s1#垂直气流方向管间距m
        self.s2=s2#沿气流方向管间距m
...
#        print(v_water,prf)
    ...
    def amendment(self,v):
            ...
            test_amendment = np.load("FCU_control/amendment.npy")
