...
# r中有diff
# 有clip(1.6)
# k=50 (0.8改成100，改成500,改成50)
# 奖励值存经验池之前除1500（原来是900）
    ...
    for i_episode in range(len(CL_DATA.columns)):  # i_episode=0-10000+
        ...
        for i in range(len(CL_DATA)):  # i = 0-13
            ...
            if s != 0:
                ...
                # a = (a.numpy()).squeeze()  #  squeeze函数压缩维度，去除所有长度为1的维度，以便得到一个一维的数组
                a = (a.numpy())

                a[0] = np.clip(np.random.normal(a[0], var_0_num), -clip_num, clip_num)  # a0 a1的值在-1.6 ~ 1.6之间
                a[1] = np.clip(np.random.normal(a[1], var_1_num), -clip_num, clip_num)
                # np.random.normal 生成一个服从正态分布的随机数，均值为a[0] ，方差为var_0_num
                ...
                a_water_list.iloc[i, 0] = a_water
                a_air_list.iloc[i, 0] = a_air
                ...
                r = -punish_coefficient_1 - (Power_fan + Power_Pump) - k * diff

                ddpg.store_transition([s_normalized, s__normalized], a, r.squeeze() / 1500,
...
if __name__ == '__main__':
    ...
    # save_path = '../contrast_experiment_result/safe_ddpg_(3)/'
    save_path = '../contrast_experiment_result/safe DDPG_reward_bar_5/第十次/'
    state_dim = 2
    actor_dim = 2
